...
WEBSITE_NAME="erp.celectronix.com"
...
sudo apt-get install git python3 python3-pip build-essential wget python3-dev python3-venv python3-wheel libxslt-dev libzip-dev libldap2-dev libsasl2-dev python3-setuptools node-less  gdebi -y
...
sudo su root -c "printf 'data_dir = $OE_HOME/odoo-data\n' >> /etc/${OE_CONFIG}.conf"
...
sudo su root -c "printf 'unaccent = $OE_UNACCENT\n' >> /etc/${OE_CONFIG}.conf"
sudo chown $OE_USER:$OE_USER /etc/${OE_CONFIG}.conf
sudo chmod 640 /etc/${OE_CONFIG}.conf

echo -e "* Create startup file"
sudo su root -c "echo '#!/bin/sh' >> $OE_HOME_EXT/start.sh"
sudo su root -c "echo 'sudo -u $OE_USER $OE_HOME/.venv/bin/python3 $OE_HOME_EXT/odoo-bin --config=/etc/${OE_CONFIG}.conf' >> $OE_HOME_EXT/start.sh"
...
EOF
  ...
  sudo mv ~/odoo /etc/nginx/sites-available/$OE_USER
